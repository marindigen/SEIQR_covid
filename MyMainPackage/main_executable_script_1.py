...
def isolation(pop):
    ...
    if grid_size % 2 == 0:
        ...
        arr1 = np.zeros((grid_size//2-1, grid_size))
        index_1 = np.random.choice(arr1.size, aff1, replace=False)
        arr1.ravel()[index_1] = 1

        # infection 1
        n = rand.randint(0, (pop - 3*grid_size)//2) # how many infected people are present
        for i in range(n):
            x = rand.randint(0, grid_size//2-2)
            y = rand.randint(0, grid_size-1)
            arr1[x][y] = 3

        # 2nd part of the array
        arr2 = np.zeros((2, grid_size))

        # hole in the middle
        arr2[:2,grid_size//2-1:grid_size//2] = 1

        # 3rd part of the array 
        arr3 = np.zeros((grid_size//2-1, grid_size))
        index_3 = np.random.choice(arr3.size, aff1, replace=False)
        arr3.ravel()[index_1] = 1
        ...
        for i in range(n):
            x = rand.randint(0, grid_size//2-2)
            y = rand.randint(0, grid_size//2-2)
            arr1[x][y] = 3

        # 2nd part of the array
        arr2 = np.zeros((3, grid_size))

        # hole in the middle
        arr2[:3,grid_size//2-1:grid_size//2] = 1

        # 3rd part of the array 
        arr3 = np.zeros((grid_size//2-1, grid_size))
        index_3 = np.random.choice(arr3.size, aff2, replace=False)
        arr3.ravel()[index_1] = 1
...
def animator(frames, ininterval, matrix, tmatrix, q_t):
    fig = plt.figure()
    ...
    plt.title('Disease progression: ' + str(q_t) + ' day(s) in quarantine')
    plt.xlabel('Time')
    ...
    pass
# np.random.randint(3, size =(50, 50))
# np.zeros((50, 50))

#matrix, tmatrix = initial_state_comp('random', 2500)
#matrix, tmatrix = initial_state('cluster', 1000)
#matrix, tmatrix = lockdown(2500)
#animator(100, 100, matrix, tmatrix, 10) 


# In[30]:


def animator_q(frames, ininterval, matrix, tmatrix, q_t):
    #fig = plt.figure()
...
def animator_on(frames, ininterval, matrix, tmatrix, q_t):
